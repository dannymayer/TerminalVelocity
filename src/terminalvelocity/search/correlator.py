...
class EventCorrelator:
    def group_by_correlation(self, events: Iterable[NormalizedEvent]) -> list[CorrelatedGroup]:
        ...
        return [CorrelatedGroup(key=key, events=sorted(group, key=lambda item: item.timestamp)) for key, group in groups.items()]
    ...
    def pivot_by_actor(self, actor: str, events: Iterable[NormalizedEvent]) -> list[NormalizedEvent]:
        return sorted([event for event in events if (event.actor or "").casefold() == actor.casefold()], key=lambda item: item.timestamp)

    def pivot_by_target(self, target: str, events: Iterable[NormalizedEvent]) -> list[NormalizedEvent]:
        return sorted([event for event in events if (event.target or "").casefold() == target.casefold()], key=lambda item: item.timestamp)
    ...
    def _is_related(seed: NormalizedEvent, event: NormalizedEvent) -> bool:
        ...
        return bool(correlation_keys.intersection({event.correlation_id, event.request_id})) or (seed.actor and event.actor and seed.actor.casefold() == event.actor.casefold()) or (seed.target and event.target and seed.target.casefold() == event.target.casefold())
