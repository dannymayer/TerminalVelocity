...
class AnomalyDetector:
    def detect(
        ...
        *,
        burst_window: timedelta = timedelta(minutes=5),
        burst_threshold: int = 5,
    ) -> list[AnomalyMarker]:
        event_list = sorted(events, key=lambda event: event.timestamp)
        return (
            self._burst_failures(event_list, burst_window, burst_threshold)
            + self._rare_actions(event_list)
            + self._privileged_operations(event_list)
        )
    ...
    def _burst_failures(self, events: list[NormalizedEvent], window: timedelta, threshold: int) -> list[AnomalyMarker]:
        ...
        for actor, actor_events in failures_by_actor.items():
            for start_index, start_event in enumerate(actor_events):
                ...
                while (
                    end_index < len(actor_events)
                    and actor_events[end_index].timestamp - start_event.timestamp <= window
                ):
                    end_index += 1
                ...
                if len(window_events) >= threshold:
                    anomalies.append(
                        AnomalyMarker(
                            kind="burst_failures",
                            description=f"{len(window_events)} failures for {actor} within {window}",
    ...
    def _rare_actions(self, events: list[NormalizedEvent]) -> list[AnomalyMarker]:
        ...
        return [
            AnomalyMarker(kind="rare_action", description=f"Rare action detected: {event.action}", events=[event])
            for event in events
            if event.action and counts[(event.provider, event.action)] == 1
        ]
            ...
            if any(term in haystack for term in PRIVILEGED_TERMS):
                markers.append(
                    AnomalyMarker(
                        ...
                        description=f"Privileged operation detected: {event.action or event.target}",
                        events=[event],
                    )
                )
        return markers
