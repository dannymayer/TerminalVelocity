...
class IdentityProtectionProvider(BaseProviderAdapter):
    ...
    async def fetch(self, start_time: datetime | None = None, end_time: datetime | None = None) -> list[NormalizedEvent]:
        ...
        risk_detections = [item async for item in self._iterate_collection(
        ...
        risky_user_filter = f"riskLastUpdatedDateTime ge {isoformat_z(start)} and riskLastUpdatedDateTime le {isoformat_z(end)}"
        risky_users = [item async for item in self._iterate_collection(
        ...
        risky_sp_filter = f"riskLastUpdatedDateTime ge {isoformat_z(start)} and riskLastUpdatedDateTime le {isoformat_z(end)}"
        risky_sps = [item async for item in self._iterate_collection(
    ...
    def normalize(self, payload: Mapping[str, Any]) -> NormalizedEvent:
        ...
        return NormalizedEvent(
            timestamp=payload.get("detectedDateTime") or payload.get("lastUpdatedDateTime") or datetime.now(UTC).isoformat(),
            provider=self.provider_name,
            service="Microsoft Entra ID Identity Protection",
            ...
            target=payload.get("ipAddress") or payload.get("location", {}).get("city") or payload.get("resourceDisplayName"),
