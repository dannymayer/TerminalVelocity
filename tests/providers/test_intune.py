...
def test_intune_fetch_normalizes_graph_audit_events(state_dir) -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if path.endswith("/deviceManagement/auditEvents"):
            return httpx.Response(200, json={"value": [{"id": "audit-1", "activityDateTime": "2025-01-01T00:10:00Z", "activityType": "Update device compliance policy", "activityResult": "success", "category": "Policy", "correlationId": "corr-2", "actor": {"userPrincipalName": "admin@contoso.com"}, "resources": [{"displayName": "Windows baseline"}]}]})
        if path.endswith("/deviceManagement/remoteActionAudits"):
            return httpx.Response(200, json={"value": [{"id": "remote-1", "actionDateTime": "2025-01-01T00:11:00Z", "actionName": "remoteLock", "actionState": "done", "managedDeviceName": "laptop-01", "userPrincipalName": "admin@contoso.com"}]})
        raise AssertionError(f"Unexpected request: {request.method} {request.url}")

    async def scenario() -> None:
        provider = IntuneProvider(tenant_id="tenant-id", client_id="client-id", client_secret="client-secret", checkpoint_store=CheckpointStore(state_dir / "checkpoints"), http_client=httpx.AsyncClient(transport=httpx.MockTransport(handler)))
