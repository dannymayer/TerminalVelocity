...
class SessionReplayTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.session = self.recorder.start_session(
            session_id="session-1", started_at=self.started_at, metadata={"provider": "entra"}
        )
        self.first = NormalizedEvent(
        ...
        self.second = NormalizedEvent(
            timestamp=self.started_at + timedelta(seconds=10),
            provider="entra",
            service="identity",
            actor="user@contoso.com",
            action="MFAChallenge",
